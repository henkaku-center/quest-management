...
contract HenkakuQuest is AccessControl {
    ...
    function addAdminRole(address _to) public {
        require(hasRole(ADMIN_ROLE, msg.sender),'You need to have an admin role to add Admin role');
        _grantRole(ADMIN_ROLE, _to);
    }

    function addQuestCreationRole(address _to) public {
        require(hasRole(ADMIN_ROLE, msg.sender),'You need to have an admin role to add quest creation role');
    ...
    function canReadQuest(address _address) public view returns(bool) {
        return hasRole(QUEST_CREATION_ROLE, _address) || hasRole(ADMIN_ROLE, _address) || hasRole(HENKAKU_MEMBER_ROLE, _address) || isMember(_address);
    }
